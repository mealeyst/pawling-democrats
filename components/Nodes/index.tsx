...
import {
  BLOCKS, Document, MARKS, Inline, Block,
...
import { ICardFields, IDeckFields, IHeroFields } from '../../@types/generated/contentful';
...
const options: Options = {
  renderMark: {
    [MARKS.BOLD]: (text) =>
      <Typography.Bold>{text}</Typography.Bold>,
    [MARKS.ITALIC]: (text) =>
      <Typography.Italic>{text}</Typography.Italic>,
  ...
  renderNode: {
    ...
    [BLOCKS.EMBEDDED_ASSET]: (node) =>
      renderEmbededAsset(node),
    [BLOCKS.EMBEDDED_ENTRY]: (node) =>
      renderEmbededEntry(node),
  },
};
